...
本地提交代码时只添加实际修改的代码文件，例如：

```powershell
git add train.py evaluate.py utils/dice_score.py visualize_prediction.py
git commit -m "Update experiment logging and IoU evaluation"
...
旧的 `log_file` 表头不再作为正式格式。旧表不要和新表混用；需要时先备份再整理：

```bash
cp experiment_results.csv experiment_results_backup_old.csv
mv experiment_results.csv experiment_results_old.csv
echo "exp_name,model,dataset,epochs,batch_size,lr,scale,amp,final_dice,final_iou,remark" > experiment_results.csv
```
...
2. 学习率实验只改 `LR`，不要同时改 `BATCH_SIZE`、`SCALE`、`EPOCHS`、`MODEL`。
